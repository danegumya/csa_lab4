...
class LispParser:
    ...
    def read_from_tokens(self):
        ...
        if token == '(':
            ...
            if self.pos >= len(self.tokens): raise SyntaxError("Missing ')'")
            self.pos += 1
            return lst
        elif token == ')':
    ...
    def build_ast(self, expr):
        if isinstance(expr, int): return {"type": "number", "value": expr}
        if isinstance(expr, dict) and expr.get("type") == "string": return expr
        if isinstance(expr, str): return {"type": "var", "name": expr}

        if isinstance(expr, list):
            if not expr: return {"type": "empty_list"}
            op = expr[0]

            if op == 'defvar': return {"type": "defvar", "name": expr[1], "expr": self.build_ast(expr[2])}
            if op == 'setq': return {"type": "setq", "name": expr[1], "expr": self.build_ast(expr[2])}

            if op in ['+', '-', 'mod', '=', '!=', '<', '>']:
                left_ast = self.build_ast(expr[1])
                for i in range(2, len(expr)):
                    right_ast = self.build_ast(expr[i])
                    left_ast = {"type": "binop", "op": op, "left": left_ast, "right": right_ast}
                return left_ast

            if op == 'if':
                return {
                    "type": "if", "cond": self.build_ast(expr[1]),
                    "then": self.build_ast(expr[2]),
                    "else": self.build_ast(expr[3]) if len(expr) > 3 else None
                }
            if op == 'defun':
                return {"type": "defun", "name": expr[1], "params": expr[2],
                        "body": [self.build_ast(s) for s in expr[3:]]}
            if op == 'funcall':
                return {"type": "funcall", "name": expr[1], "args": [self.build_ast(a) for a in expr[2:]]}

            if op == 'print': return {"type": "print", "expr": self.build_ast(expr[1])}
            if op == 'print_str': return {"type": "print_str", "expr": self.build_ast(expr[1])}
            if op == 'print_char': return {"type": "print_char", "expr": self.build_ast(expr[1])}

            if op == 'in': return {"type": "in", "port": expr[1]}
            if op == 'defirq': return {"type": "defirq", "body": [self.build_ast(s) for s in expr[1:]]}
...
class LispCompiler:
    ...
    def alloc_string(self, text):
        if text not in self.strings:
            ...
            for char in text: self.data.append(ord(char))
    ...
    def compile_expr(self, ast, is_tail=False):
        ...
        if t == "empty_list":
            ...
            if op == '+':
                ...
                jump_op = {'=': Opcode.JZ, '!=': Opcode.JNZ, '<': Opcode.JLT, '>': Opcode.JGT}[op]
            ...
            if ast["else"]: self.compile_expr(ast["else"], is_tail=is_tail)
            ...
            fname = ast["name"]
            target_params = self.functions[fname]["params"] if fname in self.functions else []
            ...
            if is_tail and fname == self.current_func:
                ...
                if fname in self.functions and self.functions[fname]["addr"] is not None:
            ...
            for stmt in ast["body"]: self.compile_expr(stmt)
...
if __name__ == '__main__':
    ...
    with open(sys.argv[2], 'wb') as f:
        f.write(binary)
    if len(sys.argv) == 4:
        with open(sys.argv[3], 'w', encoding='utf-8') as f: f.write(debug)